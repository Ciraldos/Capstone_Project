class MobileLikeScroller {
    ...
    touchstart(e) {
        if (e.button === 0) { // Check for left click
            ...
            $(document).on('dragstart', 'a', (e) => this.preventLinkDrag(e)); // Add this line
            if (this.inertialTimerInterval) {
                clearInterval(this.inertialTimerInterval);
                this.inertialTimerInterval = null;
            }
            this.childEventObject = null;
            this.blockChildrenTimeout = setTimeout(() => { this.preventChildClicks(); }, 300);
        }
    }

    preventLinkDrag(e) {
        if (this.isDragging) {
            e.preventDefault(); // Prevent default link drag behavior
        }
    }
    ...
    inertialmove() {
        var v0X = 0, v0Y = 0;
        if (this.direction != 'y') v0X = (this.previousTouchX[2] - this.previousTouchX[0]) / (this.previousTouchTime[2] - this.previousTouchTime[0]) * 1000 / $(this.target).width();
        if (this.direction != 'x') v0Y = (this.previousTouchY[2] - this.previousTouchY[0]) / (this.previousTouchTime[2] - this.previousTouchTime[0]) * 1000 / $(this.target).height();
